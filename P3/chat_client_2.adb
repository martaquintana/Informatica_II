...
exception
  ...
  when Usage_Error =>
        Ada.Text_IO.Put_Line ("./chat_client_2 <HOSTSERVER> <PORTSERVER>  <NICKNAME> ");
        LLU.Finalize;
  ...
  when Admision_Error =>
        Ada.Text_IO.Put_Line ("Mini-Chat v2.0 IGNORED new user " & ASU.To_String(Nickname) & ", nick already used");
        LLU.Finalize;
   when Ex:others =>
