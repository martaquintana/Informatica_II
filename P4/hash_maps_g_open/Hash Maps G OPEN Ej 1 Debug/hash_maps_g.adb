...
procedure Print_Map (M : Map) is
   ...
   begin
      C.M:= M;
      Ada.Text_IO.Put_Line ("Full Array Map");
      Ada.Text_IO.Put_Line ("==============");
      C.Element_A := Hash_Range'First;

      while not Fin loop
          Ada.Text_IO.Put(Hash_Range'Image(C.Element_A));
         if C.M.P_Array(C.Element_A).Cell_State = Full then
           Ada.Text_IO.Put_Line( " ["& Key_To_String(C.M.P_Array(C.Element_A).Key) & " ]");
          elsif C.M.P_Array(C.Element_A).Cell_State = Deleted_Mark then
            	Ada.Text_IO.Put_Line (" [ -- ]");
            	else
            	Ada.Text_IO.Put_Line (" [    ]");
            	end if;

              if C.Element_A /= Hash_Range'Last then
