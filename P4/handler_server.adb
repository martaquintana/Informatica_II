...
package body Handler_Server is
   ...
   function Key_To_Hash (K: ASU.Unbounded_String) return My_Hash_Range is
	    i: Natural := 1;
	    Total: Natural := 0;
  	begin
       for i in 1.. ASU.Length(K) loop
	        Total:= Total + Character'Pos(ASU.Element(K,i));
	     end loop;

	     return My_Hash_Range'Mod(Total);
   ...
   procedure Server_Handler (From : in LLU.End_Point_Type;
      ...
      Encontrado:Boolean :=False;
		  ...
		  --Subprogramas que utiliza Server_Handler
   ...
   begin
      ...
      if Mess = CM.Init then
             ...
             Active_Map.Get(Active_Clients, Nickname, Values_Active_Map, Encontrado);
						 Inactive_Map.Get(Inactive_Clients, Nickname, Values_Inactive_Map, Inactivo);
             ...
             if Acogido then
                ...
                --Print_Map(Active_Clients);
						    if Inactivo then
										--Borrar de Inactivos
										Inactive_Map.Delete(Inactive_Clients, Nickname, Encontrado);
										LLU.Reset (P_Buffer.all);

										CM.Message_Type'Output(P_Buffer,CM.Server);
										ASU.Unbounded_String'Output(P_Buffer,ASU.To_Unbounded_String("server"));
										ASU.Unbounded_String'Output(P_Buffer,ASU.To_Unbounded_String(ASU.To_String(Nickname) & " rejoins the chat"));
                ...
                --AÑADIR A LISTA DE ANTIGUOS CLIENTES
                  ...
                  --BORRAR EL MÄS ANTIGUO DE LOS ACTIVOS
